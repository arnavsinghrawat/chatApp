...
const RightSidebar = () => {
  ...
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-auto transparent-scrollbar 
      ...
      <div className='px-5 text-xs'>
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80 transparent-scrollbar'>
          {imagesDummyData.map((url,index) => {
            return <div key={index} onClick={() => window.open(url.src)} className='cursor-pointer rounded'>
              <img src={url.src} alt="" className='h-full rounded-md' />
      ...
      <button className='absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-violet-600 text-white border-none test-sm font-light py-2 px-20 rounded-full cursor-pointer'>
      Logout
      </button>
    </div>
  )
